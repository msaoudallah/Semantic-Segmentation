...
def predict_images(model, img_list, DIM, output_width, output_height, directory):
    ...
    for file in img_list:
        ...
        img = cv2.imread(str(directory)+'/' + file)
        print(img.shape)
        img = cv2.resize(img, (DIM, DIM))
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        write_output_images(pred, i, output_width, output_height)
        write_output_overlay_images(img, pred, i, output_width, output_height)
...
def write_output_images(pred, i, output_width, output_height):
    ...
    # print(rgb_mask.shape, type(rgb_mask), rgb_mask.dtype)

    cv2.imwrite('res_{}.png'.format(i), rgb_mask)


def write_output_overlay_images(x, pred, i, output_width, output_height):
    rgb_mask = np.apply_along_axis(
        map_class_to_rgb, -1, np.expand_dims(np.argmax(pred[0], axis=-1), -1))
    rgb_mask = rgb_mask.astype('float32')
    # x = x.astype('uint8')
    ...
    overlay = cv2.addWeighted(x, 0.7, rgb_mask, 0.5, 1)
    cv2.imwrite('overlay_res_{}.png'.format(i), overlay)


def encode_labels(img):
    for k in mapping_20:
        r1, g1, b1 = k[0]  # Original value
        r2, g2, b2 = k[1]  # Value that we want to replace it with
...
def predict_images(model, img_list, DIM, output_width, output_height, output_images_dir, directory):
    i = 0
    for file in img_list:
        i += 1
        x = np.array(
        ...
        img = cv2.resize(img, (DIM, DIM))
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        write_output_images(pred, i, output_width,
                            output_height, output_images_dir)
        write_output_overlay_images(
            img, pred, i, output_width, output_height, output_images_dir)


def write_output_images(pred, i, output_width, output_height, output_images_dir):
    rgb_mask = np.apply_along_axis(
        map_class_to_rgb, -1, np.expand_dims(np.argmax(pred[0], axis=-1), -1))
    rgb_mask = rgb_mask.astype('float32')
    # x = x.astype('float32')
    ...
    cv2.imwrite(os.path.join(output_images_dir,
                             'res_{}.png'.format(i)), rgb_mask)


def write_output_overlay_images(x, pred, i, output_width, output_height, output_images_dir):
    rgb_mask = np.apply_along_axis(
        map_class_to_rgb, -1, np.expand_dims(np.argmax(pred[0], axis=-1), -1))
    rgb_mask = rgb_mask.astype('float32')
    ...
    overlay = cv2.addWeighted(x, 0.7, rgb_mask, 0.3, 1)
    overlay = cv2.resize(overlay, (output_width, output_height))

    cv2.imwrite(os.path.join(output_images_dir,
                             'overlay_res_{}.png'.format(i)), overlay)
