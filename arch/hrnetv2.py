...
class HRModule(layers.Layer):
    ...
    def _make_branch(self, branch_level):
        blocks = []
        branch_name = 'hrm_{}_module{}_branch{}'.format(
            self.stage_name, self.module_idx, branch_level)
            ...
            for j in range(self.num_branches):
                if j > i:  # upsample low to high
                    ...
                    upsample = layers.UpSampling2D(
                        size=(2**(j-i), 2**(j-i)), interpolation='nearest')
                    fuse_layer.append(
                        tf.keras.Sequential([conv_mod, upsample]))
                elif j == i:
                    ...
                    for k in range(i-j):
                        if k == i-j-1:
                            down_name = 'fusedown_{}_{}_{}'.format(
                                self.stage_name, j, i)
                            conv_mod = ConvModule(
                                self.num_channels[i],
                                3,
        ...
        for i in range(self.num_branches):
            if i < prev_layer_branches:
                ...
                self.transition_layers.append(
                    tf.keras.Sequential(new_transitions))
...
class ClsHead(layers.Layer):
    ...
    def call(self, x_list, training=None):
        ...
        for i in range(1, len(self.downsample_layers)+1):
            y = self.width_incr_layers[i](
                x_list[i], training=training) + self.downsample_layers[i-1](y, training=training)
    ...
    def __init__(self, model_cfg):
        ...
        for s in range(1, model_cfg['num_stages'] + 1):
            ...
            if s == 1:
                ...
                prev_layer_channels = [
                    prev_stage_cfg['expansion'] * c for c in prev_stage_cfg['num_channels']]
                trans_name = 'transition_{}_{}'.format(s-1, s)
                self.transitions.append(Transition(
                    stage_cfg, prev_layer_branches, prev_layer_channels, name=trans_name))
