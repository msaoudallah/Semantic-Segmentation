...
def conv_block(X, filters, block):
    ...
    # skip_conv
    X_skip = Convolution2D(filters=f3, kernel_size=(
        3, 3), padding='same', name=b+'skip_conv')(X_skip)
...
def pyramid_feature_maps(input_layer):
    ...
    red = UpSampling2D(size=256, interpolation='bilinear',
    ...
    green = UpSampling2D(size=8, interpolation='bilinear',
...
def last_conv_module(input_layer):
    ...
    X = Convolution2D(filters=3, kernel_size=3,
                      padding='same', name='last_conv_3_by_3')(X)
...
def model(DIM,):
    input_layer = tf.keras.Input(shape=(DIM, DIM, 3), name='input')
